...
import { User, FileText, ChevronRight, Stethoscope, Users, ArrowLeft } from 'lucide-react';
...
const DoctorList: React.FC<DoctorListProps> = ({ onSelectDoctor, labId, labName, onBack }) => {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">
            {labName ? `${labName}: Doctor Registry` : 'Referring Doctors'}
          </h2>
          <p className="text-sm text-slate-500">
            {labName ? 'Physicians associated with this laboratory' : 'Select a physician to view their patient records'}
          </p>
        </div>
        <div className="p-2 bg-slate-100 rounded-md text-slate-500 flex items-center text-sm font-medium">
